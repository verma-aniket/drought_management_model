...
# Notes July 11, 2026
# the simulation model is VERY sensitive to turbiity!!!
# turn all turbidity flags off (assume all NC, tait supply can be used) and Felton is barely needed (unmet demand = 1872.52 MG)
# turn all tubidity flags on (assume all NC, tait water is turbid and cannot be used) and Felton is throttled always (unmet demand = 91499.95 MG)
# remember to check turbidity flags before running!!!
# the base modl determined turbidity from watershed specific precip data
# we only have watershed average precip so turbidity flags are more likely to be off, which means NC, tait water will be more accessible,
# meaning unmet demand will be lower, need to be aware of this limitation

# here is the control curve from the pywr model JSON file

        # "loch_lomond_water_value": {                                      ### loch_lomond_water_value is the cost of using LL water
        #     "type": "controlcurveinterpolated",
        #     "storage_node": "ll_reservoir",
        #     "comment": "with the water stored in the ASR reservoir",
        #     "control_curves": [                                           ### control_curves is the fraction of reservoir that is full 
        #         0.93742,
        #         0.87483,
        #         0.81225,
        #         0.74966,
        ...
        #         0.49933,
        #         0.43674,
        #         0.37416,                                                  ### LL_reserve/LL_max = 1070/2859.737 = 0.37416028117
        #         0.37415,                                                  ### "just below" the reserve
        #         0.1                                                       ### 10% of filling
        #     ],
        #     "values": [
        #         -10.01,
        ...
        #         -10.91,
        #         -11.01,
        #         -1300,                                                    ### the costs here are all if the reservoir volume is 
        #         -1300,                                                    ###     at the reserve volume or below
        #         -1300
        #     ]
        # },

# need to find a way to translate this hedging policy into the Python model, go with some simple

class WaterBalanceModel:
    def __init__(self, params_dict, profiles_dict, hydro_types_dict, bathymetry_df, env_flows_dict, flow_df, weather_df, demand_df):
        ...
        # store all flow and weather time series data
        ...
        # Notes:     
        #   - Working with numpy arrays instead of dataframes for dynamic data handling 
        #   - will keep track of a baseline demand that is unaltered
        #   - simulation will use active demand that can be altered based on policy actions
        #   - demand value look-ups are based on a year-month key mapped to numpy array index
        ...
        unique_months = self.flow[['year', 'month']].drop_duplicates().sort_values(['year', 'month']).reset_index(drop=True)

        # 1. Map (year, month) -> sequential month index for instant internal lookups
        self.month_to_idx = {tuple(x): i for i, x in enumerate(unique_months.values)}

        # 2. Pre-allocate the Baseline and Endogenous Demand tracks (in MG per month)
        self.demand_baseline = np.zeros(len(unique_months))

        for (yr, mn), idx in self.month_to_idx.items():
            # Grab original baseline projection
            base_val = self.demand.loc[(self.demand['year'] == yr) & (self.demand['month'] == mn), 'demand_MG'].values
            self.demand_baseline[idx] = float(base_val[0]) if len(base_val) > 0 else 0.0

        # 3. Create the mutable tracking array that policies will modify
        self.demand_active = self.demand_baseline.copy()
    ...
    def get_critically_dry(self):
        ...
        flow_df = self.flow[['year', 'month', 'bigtrees']].copy()
    ...
    def get_turbidity(self):
        ...
        SLR_flag[1:] += event[:-1]
        SLR_flag[2:] += event[:-2]
        self.slr_turb = np.array(self.weather['slr_turb'].values, dtype=np.int8) # np.zeros(len(precip)) # 1 - SLR_flag # np.ones(len(precip))
        
        # Liddel and Laguna turbidity flag
        NC_flag = event.copy()
        NC_flag[1:] += event[:-1]
        self.nc_turb = np.array(self.weather['nc_turb'].values, dtype=np.int8) # np.zeros(len(precip)) # 1 - NC_flag # np.ones(len(precip))
        ...
        self.maj_turb = np.array(self.weather['maj_turb'].values, dtype=np.int8) # np.zeros(len(precip)) # 1 - MAJ_flag # np.ones(len(precip))
    ...
    def get_NC_supply(self, row: pd.Series) -> tuple:
        """
        ...
        monthly_fraction = float(self.p_NC_farm.get(int(row.get('month')), 0.0)) / 100 # Look up the monthly allocation fraction from profiles
        farmer_monthly_demand = monthly_fraction * self.d_NC_Farm

        # Covert monthly demand to daily demand
        farmer_demand = self.get_daily_demand(farmer_monthly_demand, int(row.get('year')), int(row.get('month')))
        ...
        # - pywr's cost min based flow routing changes how much water is extracted from 
        # the NC creeks, causing differences in the total supply, leading to differences in unmet demand
        # limitation - Python, min/max "all or nothing", no in-between cost minimization routing like in pywr
        # s_lid = float(row.get('s_lid', 0.0))
        # s_lag = float(row.get('s_lag', 0.0))
        # s_maj = float(row.get('s_maj', 0.0))
        ...
        return s_NC, unmet_farmer_demand, met_farmer_demand, s_lid, s_lag, s_maj
    ...
    def get_tait_supply(self, row: pd.Series, i_slr_felton: float) -> float:
        """
    ...
    def route_san_lorenzo_river(self, row: pd.Series, s_NC: float, s_b12: float, s_oakwell: float, 
        ...
        if self.is_felton_pump_on and (self.first_flush) and (t_slr == 1) and (v_tot < self.LL_reserve):
            ...
            s_tait, s_tait_div = self.get_tait_supply(row, i_slr_felton_actual)
            ...
            while True:
                ...
                s_tait, s_tait_div = self.get_tait_supply(row, i_slr_felton_guess)
                ...
                if iteration_counter >= max_iterations:
                    print(f"Convergence warning on {int(row['year'])}-{int(row['month'])}-{int(row['day'])}: Loop capped out at {max_iterations} iterations.")
                    break
    ...
    def run_simulation(self):
        ...
        for t in tqdm(range(num_days), desc="Day", ncols=100, colour="green", mininterval=1, maxinterval=5):
            ...
            # A. Get total urban demand for today (Disaggregating monthly baseline to daily)
            current_month_idx = self.month_to_idx[(year, month)]                    # identify which sequential month index we are currently in
            d_urban_monthly = self.demand_active[current_month_idx]                 # pull the dynamically updated active monthly demand
            d_urban_daily = self.get_daily_demand(d_urban_monthly, year, month)     # disaggregate down to today's daily allocation target

            # B. Gather North Coast Pipeline surface supplies and 
            # Beltz 12 Well and Live Oak Wells groundwater supplies
            s_NC, unmet_farm, _, _, _, _ = self.get_NC_supply(row)
            s_b12 = self.get_b12_supply(month)
            s_oakwell = self.get_oakwell_supply(month)
            # s_oakwell = self.flow['c_tait'].values[t] # using pywr oakwell flows does not impact the model at all
            ...
            slr_routing = self.route_san_lorenzo_river(row, s_NC, s_b12, s_oakwell, d_urban_daily, LL_space, hyd_const)
            ...
            # B. Save results to pre-allocated output arrays
            outputs["s_NC"][t] = s_NC
        ...
        simulation_results_df = pd.DataFrame(outputs, index=self.flow.index)
        
        # Attach calendar date context columns from the input data for easy plotting
        simulation_results_df['year'] = self.flow['year']
        simulation_results_df['month'] = self.flow['month']
        simulation_results_df['day'] = self.flow['day']
